...
describe('AgentMeshError', () => {
  it('should set message, code, and name', () => {
    const err = new AgentMeshError('test error', 'TEST_CODE');
    expect(err.message).toBe('test error');
    expect(err.code).toBe('TEST_CODE');
    expect(err.name).toBe('AgentMeshError');
    expect(err instanceof Error).toBe(true);
  });

  it('should store cause', () => {
    const cause = new Error('root cause');
    const err = new AgentMeshError('wrapper', 'WRAP', cause);
...
describe('EscrowNotFoundError', () => {
  ...
  it('should be an instance of AgentMeshError', () => {
    const err = new EscrowNotFoundError('1');
    expect(err instanceof AgentMeshError).toBe(true);
    expect(err instanceof Error).toBe(true);
  });
});
...
describe('RegistrationError', () => {
  it('should store DID', () => {
    const err = new RegistrationError('already registered', 'did:agentmesh:base:agent1');
    expect(err.did).toBe('did:agentmesh:base:agent1');
...
describe('Result pattern', () => {
  ...
  describe('isSuccess()', () => {
    ...
    it('should return false for failure result', () => {
      const result: Result<number> = failure(new AgentMeshError('err', 'ERR'));
  ...
  describe('type narrowing', () => {
    it('should narrow to value on isSuccess', () => {
      const result: Result<string, AgentMeshError> = success('hello');
      if (isSuccess(result)) {
