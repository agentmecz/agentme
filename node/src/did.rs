//! W3C DID Document support for AgentMesh.
//!
...
pub const DID_METHOD: &str = "agentmesh";
...
/// AgentMesh-specific metadata in DID Document.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl DIDDocumentBuilder {
    ...
    pub fn add_capability_card_service(mut self, url: &str) -> Self {
        ...
        self.services.push(ServiceEndpoint {
            ...
            service_endpoint: url.to_string(),
            description: Some("AgentMesh Capability Card".to_string()),
        });
        self
    }
...
impl DIDDocument {
    ...
    ///
    /// Format: `did:agentmesh:{chain}:{identifier}`
    pub fn parse_did(did: &str) -> Result<(String, String, String)> {
        let parts: Vec<&str> = did.split(':').collect();
        if parts.len() != 4 {
            return Err(Error::Did(format!(
                "Invalid DID format: '{}'. Expected did:agentmesh:chain:identifier",
                did
            )));
        }
        if parts[0] != "did" {
...
mod tests {
    ...
    fn test_parse_did_valid() {
        let result = DIDDocument::parse_did("did:agentmesh:base:abc123");

        assert!(result.is_ok());
        let (method, chain, identifier) = result.unwrap();
        assert_eq!(method, "agentmesh");
    ...
    fn test_parse_did_invalid_format() {
        let result = DIDDocument::parse_did("did:agentmesh:base");
    ...
    fn test_parse_did_wrong_scheme() {
        let result = DIDDocument::parse_did("urn:agentmesh:base:abc");
    ...
    fn test_builder_creates_valid_document() {
        ...
        assert!(doc.is_ok());
        let doc = doc.unwrap();
        assert_eq!(doc.id, "did:agentmesh:base:test-agent");
        assert!(doc
            .context
    ...
    fn test_validate_detects_invalid_verification_method_controller() {
        ...
        doc.verification_method = Some(vec![VerificationMethod {
            id: "did:agentmesh:base:test#key-1".to_string(),
            method_type: "Ed25519VerificationKey2020".to_string(),
            controller: "did:agentmesh:base:other".to_string(), // Wrong controller
    ...
    fn test_resolution_not_found() {
        let result = DIDResolutionResult::not_found("did:agentmesh:base:unknown");
