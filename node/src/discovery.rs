...
pub struct CapabilityCard {
    ...
    /// AgentMesh-specific extensions.
    #[serde(rename = "x-agentmesh")]
    pub agentmesh: Option<AgentMeshExtension>,
}
...
/// AgentMesh-specific extensions to the Capability Card.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct AgentMeshExtension {
    /// Agent's DID.
    pub did: String,
...
impl DiscoveryService {
    ...
    ///
    /// Returns an error if:
    /// - The card is missing the AgentMesh extension with DID
    /// - The DID format is invalid
    pub async fn register(&self, card: &CapabilityCard) -> Result<()> {
        // Validate: card must have agentmesh extension with DID
        let agentmesh = card
            .agentmesh
            .as_ref()
            .ok_or_else(|| Error::Discovery("Missing agentmesh extension with DID".to_string()))?;

        let did = &agentmesh.did;
        ...
        if let Some(ref tx) = self.network_tx {
            ...
            tx.send(SwarmCommand::Publish {
                topic: "/agentmesh/discovery/1.0.0".to_string(),
                data: serialized,
            })
            .await
...
mod tests {
    ...
    async fn test_register_valid_capability_card() {
        ...
        let card = sample_capability_card("did:agentmesh:base:test-agent-123");
    ...
    #[tokio::test]
    async fn test_register_requires_did_in_agentmesh_extension() {
        // Arrange
        let service = DiscoveryService::new();
        let mut card = sample_capability_card("did:agentmesh:base:test");
        card.agentmesh = None; // Remove agentmesh extension
        ...
        assert!(
            err.to_string().contains("DID") || err.to_string().contains("agentmesh"),
            "Error should mention missing DID or agentmesh extension"
        );
    }
    ...
    async fn test_register_stores_card_for_later_retrieval() {
        // Arrange
        let service = DiscoveryService::new();
        let did = "did:agentmesh:base:retrievable-agent";
    ...
    async fn test_get_returns_none_for_unknown_did() {
        ...
        // Act
        let result = service.get("did:agentmesh:base:nonexistent").await;
    ...
    async fn test_get_returns_correct_card_among_multiple() {
        // Arrange
        let service = DiscoveryService::new();
        let did1 = "did:agentmesh:base:agent-1";
        let did2 = "did:agentmesh:base:agent-2";
        let did3 = "did:agentmesh:base:agent-3";
    ...
    async fn test_search_returns_empty_for_no_matches() {
        // Arrange
        let service = DiscoveryService::new();
        let card = sample_capability_card("did:agentmesh:base:translator");
    ...
    async fn test_search_finds_agents_by_name() {
        // Arrange
        let service = DiscoveryService::new();
        let mut card1 = sample_capability_card("did:agentmesh:base:translator-1");
        card1.name = "French Translator".to_string();
        let mut card2 = sample_capability_card("did:agentmesh:base:coder-1");
        card2.name = "Code Assistant".to_string();
        let mut card3 = sample_capability_card("did:agentmesh:base:translator-2");
    ...
    async fn test_search_finds_agents_by_capability() {
        // Arrange
        let service = DiscoveryService::new();
        let card = sample_capability_card("did:agentmesh:base:agent");
        service.register(&card).await.unwrap();
    ...
    async fn test_search_is_case_insensitive() {
        // Arrange
        let service = DiscoveryService::new();
        let mut card = sample_capability_card("did:agentmesh:base:agent");
    ...
    async fn test_register_sends_dht_put_command_when_network_available() {
        ...
        let did = "did:agentmesh:base:dht-test-agent";
        let card = sample_capability_card(did);
    ...
    async fn test_register_still_works_without_network() {
        ...
        let service = DiscoveryService::new();
        let did = "did:agentmesh:base:offline-agent";
        let card = sample_capability_card(did);
    ...
    async fn test_get_queries_dht_when_not_in_cache() {
        ...
        let (tx, mut rx) = mpsc::channel::<SwarmCommand>(10);
        let service = DiscoveryService::with_network(tx);
        let did = "did:agentmesh:base:remote-agent";
    ...
    async fn test_get_returns_card_from_dht_response() {
        ...
        let (tx, mut rx) = mpsc::channel::<SwarmCommand>(10);
        let service = DiscoveryService::with_network(tx);
        let did = "did:agentmesh:base:dht-stored-agent";
        let card = sample_capability_card(did);
    ...
    async fn test_get_returns_none_when_dht_has_no_record() {
        ...
        let (tx, mut rx) = mpsc::channel::<SwarmCommand>(10);
        let service = DiscoveryService::with_network(tx);
        let did = "did:agentmesh:base:nonexistent-dht-agent";
    ...
    async fn test_get_caches_card_from_dht_response() {
        ...
        let did = "did:agentmesh:base:cache-from-dht-agent";
        let card = sample_capability_card(did);
        let serialized_card = serde_json::to_vec(&card).unwrap();
    ...
    async fn test_get_returns_cached_value_without_dht_query() {
        ...
        let (tx, mut rx) = mpsc::channel::<SwarmCommand>(10);
        let service = DiscoveryService::with_network(tx);
        let did = "did:agentmesh:base:cached-agent";
        let card = sample_capability_card(did);
    ...
    async fn test_register_sends_gossipsub_announce_when_network_available() {
        ...
        let (tx, mut rx) = mpsc::channel::<SwarmCommand>(10);
        let service = DiscoveryService::with_network(tx);
        let did = "did:agentmesh:base:announced-agent";
        let card = sample_capability_card(did);
    ...
    async fn test_search_ranks_results_by_trust_score() {
        ...
        let mut low_trust = sample_capability_card("did:agentmesh:base:low-trust");
        low_trust.name = "Translation Agent Low".to_string();
        if let Some(ref mut ext) = low_trust.agentmesh {
            ext.trust_score = Some(0.3);
        }

        let mut high_trust = sample_capability_card("did:agentmesh:base:high-trust");
        high_trust.name = "Translation Agent High".to_string();
        if let Some(ref mut ext) = high_trust.agentmesh {
            ext.trust_score = Some(0.9);
        }

        let mut medium_trust = sample_capability_card("did:agentmesh:base:medium-trust");
        medium_trust.name = "Translation Agent Medium".to_string();
        if let Some(ref mut ext) = medium_trust.agentmesh {
        ...
        let scores: Vec<f64> = results
            .iter()
            .filter_map(|c| c.agentmesh.as_ref().and_then(|e| e.trust_score))
            .collect();
    ...
    async fn test_cache_size_reflects_registered_agents() {
        ...
        service
            .register(&sample_capability_card("did:agentmesh:base:agent1"))
            .await
            .unwrap();
        assert_eq!(service.cache_size(), 1);

        service
            .register(&sample_capability_card("did:agentmesh:base:agent2"))
            .await
            .unwrap();
    ...
    async fn test_register_indexes_card_in_hybrid_search() {
        ...
        let card = sample_capability_card("did:agentmesh:base:indexed-agent");
        ...
        // Verify card is in local cache
        let cached = service.get("did:agentmesh:base:indexed-agent").await;
        assert!(cached.is_ok());
        assert!(
    ...
    async fn test_search_uses_hybrid_search_when_available() {
        ...
        let mut card = sample_capability_card("did:agentmesh:base:code-reviewer");
        ...
        assert_eq!(
            matches[0]
                ...
                .map(|e| e.did.as_str())
                .unwrap_or(""),
            "did:agentmesh:base:code-reviewer"
        );
    }
    ...
    async fn test_search_falls_back_to_simple_search_without_hybrid() {
        ...
        // Register a card
        let mut card = sample_capability_card("did:agentmesh:base:simple-agent");
    ...
    async fn test_hybrid_search_ranks_by_relevance_not_trust_score() {
        ...
        let mut high_relevance = sample_capability_card("did:agentmesh:base:high-relevance");
        ...
        if let Some(ref mut ext) = high_relevance.agentmesh {
            ext.trust_score = Some(0.3); // Low trust
        }

        // Create an agent that matches poorly but has high trust score
        let mut low_relevance = sample_capability_card("did:agentmesh:base:low-relevance");
        ...
        if let Some(ref mut ext) = low_relevance.agentmesh {
        ...
        if !results.is_empty() {
            let first_did = results[0]
                ...
                .unwrap_or("");
            assert_eq!(
                first_did, "did:agentmesh:base:high-relevance",
    ...
    async fn test_with_network_and_search_has_both_capabilities() {
        ...
        let card = sample_capability_card("did:agentmesh:base:combined-test");
        service.register(&card).await.unwrap();
