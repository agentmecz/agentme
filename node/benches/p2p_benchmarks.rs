...
//!
//! Benchmarks for measuring AgentMesh P2P network performance:
...
fn bench_message_serialization(c: &mut Criterion) {
    ...
    let card = create_sample_card("did:agentmesh:base:benchmark-agent");
    ...
    for batch_size in [10, 100, 1000].iter() {
        let messages: Vec<_> = (0..*batch_size)
            .map(|i| TrustMessage::ReputationEvent {
                did: format!("did:agentmesh:base:agent-{}", i),
                success: i % 2 == 0,
...
fn bench_discovery_operations(c: &mut Criterion) {
    ...
    // Create sample capability card
    let card = create_sample_card("did:agentmesh:base:test");
    ...
    let cards: Vec<_> = (0..1000)
        .map(|i| create_sample_card(&format!("did:agentmesh:base:agent-{}", i)))
        .collect();
...
fn bench_trust_cache(c: &mut Criterion) {
    ...
    group.bench_function("cache_insert", |b| {
        ...
        b.iter(|| {
            counter += 1;
            let did = format!("did:agentmesh:base:agent-{}", counter);
            let info = TrustInfo {
                did: did.clone(),
                score: 0.85,
                reputation: 0.8,
    ...
    runtime.block_on(async {
        for i in 0..1000 {
            let did = format!("did:agentmesh:base:lookup-{}", i);
            let info = TrustInfo {
                did: did.clone(),
    ...
    group.bench_function("cache_lookup_hit", |b| {
        b.iter(|| {
            runtime.block_on(async {
                let result = cache.get("did:agentmesh:base:lookup-500").await;
                black_box(result);
            });
        });
    });
    ...
    group.bench_function("cache_lookup_miss", |b| {
        b.iter(|| {
            runtime.block_on(async {
                let result = cache.get("did:agentmesh:base:nonexistent").await;
                black_box(result);
            });
        });
    });
...
fn bench_juror_pool(c: &mut Criterion) {
    ...
    for i in 0u64..1000 {
        ...
        pool.register_juror(format!("did:agentmesh:base:juror-{}", i), stake, vec![0])
            .unwrap();
    }
    ...
    group.bench_function("register_juror", |b| {
        ...
        b.iter(|| {
            ...
            let _ = fresh_pool.register_juror(
                format!("did:agentmesh:base:fresh-juror-{}", counter),
                500_000_000,
                vec![0],
            );
        });
    ...
    group.bench_function("get_juror", |b| {
        b.iter(|| {
            let juror = pool.get_juror("did:agentmesh:base:juror-500");
